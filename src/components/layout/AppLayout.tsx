  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
          <header className="sticky top-0 z-20 flex min-h-[4.5rem] items-center justify-between gap-3 border-b border-white/60 bg-white/70 px-4 py-2 backdrop-blur-xl sm:px-6">
             ...
             <div className="flex items-center gap-1">
               <Link
                 ...
                 aria-label="Laman Tempahan"
                  className="inline-flex h-11 items-center gap-1.5 rounded-full px-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground sm:h-9 sm:px-3"
               >
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                   <Button variant="ghost" className="h-11 gap-2 px-1.5 sm:px-2">
                     ...
                     <span className="hidden max-w-[130px] truncate text-sm font-medium sm:inline">{displayName}</span>
                  </Button>
                </DropdownMenuTrigger>
      ...
      <nav
        ...
        className="fixed inset-x-0 bottom-0 z-40 border-t bg-background/95 pb-[env(safe-area-inset-bottom)] shadow-[0_-1px_0_rgba(0,0,0,0.05)] backdrop-blur lg:hidden"
      >
        <div className="relative w-full">
          <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-8 bg-gradient-to-l from-background/95 to-transparent" aria-hidden="true" />
          <div className="flex w-full touch-pan-x overflow-x-auto overscroll-x-contain px-1 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
          <div className="flex min-w-max">
          <NavLink
            to="/"
            aria-label="Laman Tempahan"
            className="flex min-h-16 w-[88px] shrink-0 flex-col items-center justify-center gap-1 px-2 py-2 text-[10px] font-medium text-muted-foreground transition-colors hover:text-foreground"
          >
          ...
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  'flex min-h-16 w-[88px] shrink-0 flex-col items-center justify-center gap-1 px-2 py-2 text-[10px] font-medium transition-colors',
